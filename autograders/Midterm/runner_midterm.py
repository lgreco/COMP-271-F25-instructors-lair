...
def rename_submission(path: Path, dest_dir: Path):
    ...
    if len(pdf_files) > 1:
        raise ValueError(f"Multiple PDF files found in {path}: {pdf_files}")
    elif len(pdf_files) == 0:
        raise ValueError(f"No PDF files found in {path}")

    src_pdf_file = pdf_files[0]
    dest_pdf_file = dest_dir / f"pdfs/{uvid}_writeup.pdf"

    if not dest_pdf_file.exists():
        ## copy the source pdf to dest pdf
        logger.info(f"Copying {src_pdf_file} to {dest_pdf_file}")
        shutil.copy(src_pdf_file, dest_pdf_file)
...
def main():
    sakai_submissions_dir = Path('./Sakai-Midterm')  ## example
...
if __name__ == "__main__":
    main()
    # grade_single_submission(name='Dawson', uvid='awatt2')
