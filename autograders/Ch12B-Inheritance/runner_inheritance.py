...
def rename_submission(path: Path, dest_dir: Path):
    """
    Check for 4 .py files and copy to dest_dir
    """
    ...
    check_set = {'arya', 'brienne', 'oberyn', 'tyrion'}
    py_filenames = {py_file.stem.lower() for py_file in py_files}
    assert py_filenames.issuperset(check_set), f'{py_filenames=}'

    for src_file in py_files:
        dest_file = dest_dir / f"{uvid}_{ASSIGNNMENT_NAME}_{src_file.stem.lower()}.py"
    ...
    for i, student_dir in enumerate(sorted(sakai_basedir.iterdir())):
        ...
        if student_dir.is_dir():
            try:
                ...
                uvid, name = rename_submission(py_path, dest_dir)
...
def grade_submissions(submission_dir: Path, uvid2name_map: dict):
    ...
    for uvid, name in tqdm(uvid2name_map.items()):
    # for py_file in sorted(submission_dir.glob("*_Triangle.py")):
    #     uvid = py_file.stem.split("_")[0]
    #     name = uvid2name_map.get(uvid, "Unknown")
        logger.info(f"Grading {uvid!r}")
    ...
    # Write summary CSV
    summary_csv = "grades_summary.csv"
    ...
    sorted_csv = "grades_summary_sorted.csv"
...
def main():
    # sakai_submissions_dir = Path('./Sakai-12B-Inherit')  ## example
...
if __name__ == "__main__":
    main()
    # grade_single_submission(name='Nova', uvid='ykurul')  # to grade a single submission
